  ...
  func importFlights(from text: String, columnMapping: ImportColumnMapping) -> [Flight] {
    ...
    for line in lines {
        ...
        arrIndex < components.count
      {
        ...
        let departureAirport = db.getAirportByCode(depCode)
        let arrivalAirport = db.getAirportByCode(arrCode)
      ...
      // Parse aircraft registration and type
      var aircraftRegistration: String?

      if let regIndex = columnMapping.getColumnIndex(for: .aircraftRegistration),
        regIndex < components.count
      {
        aircraftRegistration = components[regIndex]
      }

      // Look up or create aircraft
      if let registration = aircraftRegistration {
        if let existingAircraft = db.getAircraftByRegistration(registration) {
          // Use existing aircraft
          flight.aircraft = existingAircraft
          flight.aircraftId = existingAircraft.id
        } else {
          // Create new aircraft with default type
          let defaultType = "B77W"  // Default type for new aircraft
          let newAircraft = Aircraft(registration: registration, type: defaultType)
          do {
            try db.createAircraft(newAircraft)
            // Fetch the newly created aircraft to get its ID
            if let createdAircraft = db.getAircraftByRegistration(registration) {
              flight.aircraft = createdAircraft
              flight.aircraftId = createdAircraft.id
            } else {
              print("Error: Could not fetch created aircraft")
              continue
            }
          } catch {
            print("Error creating aircraft: \(error)")
            continue  // Skip this flight if aircraft creation fails
          }
        }
      } else {
        print("Error: Aircraft registration required")
        continue  // Skip this flight if registration is missing
      }
      ...
      if let opCapIndex = columnMapping.getColumnIndex(for: .operatingCapacity),
        opCapIndex < components.count
      {
