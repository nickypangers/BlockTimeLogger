...
class ExportService {
    ...
    private func filterFlightsByDateRange(_ flights: [Flight], dateRange: Export.DateRange)
        -> [Flight]
    {
        ...
        case .lastMonth:
            ...
            let previousMonthComponents = calendar.date(
                byAdding: .month, value: -1, to: firstDayOfCurrentMonth
            )!
            let firstDayOfPreviousMonth = calendar.date(
                from: calendar.dateComponents([.year, .month], from: previousMonthComponents))!

            // Get the first day of the current month (which is the end of the previous month)
            let firstDayOfNextMonth = calendar.date(
                byAdding: .month, value: 1, to: firstDayOfPreviousMonth
            )!
    ...
    private func generatePDF(
        flights: [Flight], style: Export.LogbookStyle,
        completion: @escaping (Result<URL, Error>) -> Void
    ) {
        ...
        do {
            try renderer.writePDF(to: tempURL) { context in
                ...
                let tableWidth =
                    columnWidths.reduce(0, +) + (CGFloat(columnWidths.count - 1) * columnSpacing)
                ...
                for flight in sortedFlights {
                    ...
                    let needsNewPage = flightCount >= 26 || scaledYPosition > availableHeight - 50
                    ...
                    // Prepare flight entries
                    let entries = prepareFlightEntries(
                        flight: flight,
                ...
                let scaledYPosition = yPosition * widthScaleFactor
                let needsNewPageForTotals = scaledYPosition + spaceNeededForTotals > availableHeight - 50

                if needsNewPageForTotals {
                    context.beginPage()
                    yPosition = 0
                ...
                let p1usTotal = flights.filter { $0.operatingCapacity == .p1us }.reduce(0) {
                    $0 + $1.blockTime
                }
                let p2Total = flights.filter { $0.operatingCapacity == .p2 || $0.operatingCapacity == .p2x }
                    .reduce(0) { $0 + $1.blockTime }
                let putTotal = flights.filter { $0.operatingCapacity == .put }.reduce(0) {
                    $0 + $1.blockTime
                }
    ...
    private func prepareFlightEntries(flight: Flight, dateFormatter: DateFormatter) -> [String] {
        return [
            ...
            flight.operatingCapacity == .p2 || flight.operatingCapacity == .p2x
    ...
    private func drawFlightEntries(
        entries: [String],
        xPosition: inout CGFloat,
        yPosition: inout CGFloat,
        columnWidths: [CGFloat],
    ...
    private func generateCSV(
        ...
        for flight in flights {
            content +=
                "\(flight.formattedDate),\(flight.aircraft?.type ?? ""),\(String(describing: flight.departureAirport)),\(String(describing: flight.arrivalAirport)),\(String(format: "%.1f", flight.blockTime)),\n"
        }
