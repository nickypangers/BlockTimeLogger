              ...
              Text(option.rawValue)
                ...
                .background(
                  RoundedRectangle(cornerRadius: 16)
                    .fill(
                      filterOption == option ? Color.accentColor : Color(.secondarySystemBackground)
                    )
                )
    ...
    if !searchText.isEmpty {
      result = result.filter { flight in
        flight.departureAirportICAO.localizedCaseInsensitiveContains(searchText)
          ...
          || flight.aircraft?.registration.localizedCaseInsensitiveContains(searchText) ?? false
          || flight.aircraft?.type.localizedCaseInsensitiveContains(searchText) ?? false
      }
    }
