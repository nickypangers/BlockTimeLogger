...
extension LocalDatabase {
  ...
  private func loadAirportsFromJSON() async {
    do {
      ...
      print(
        "LocalDatabase: Successfully loaded \(successCount) out of \(totalCount) airports from JSON"
      )
    } catch {
    ...
    do {
      return try reader.read { db in
        try Airport
          .filter(
            sql:
              "\(Airport.Columns.name.rawValue) LIKE ? OR \(Airport.Columns.icao.rawValue) LIKE ? OR \(Airport.Columns.iata.rawValue) LIKE ?",
...
extension LocalDatabase {
  ...
  func getAircraftByRegistration(_ registration: String) -> Aircraft? {
    do {
      return try reader.read { db in
        try Aircraft.filter(Aircraft.Columns.registration == registration).fetchOne(db)
      }
    } catch {
      print("LocalDatabase: Error fetching aircraft by registration \(registration): \(error)")
      return nil
    }
  }

  func getAircraftById(_ id: Int) -> Aircraft? {
    do {
      return try reader.read { db in
        try Aircraft.filter(Aircraft.Columns.id == id).fetchOne(db)
      }
    } catch {
      print("LocalDatabase: Error fetching aircraft by id \(id): \(error)")
      return nil
    }
  }
  ...
  func getAllAircraft() -> [Aircraft] {
    do {
      return try reader.read { db in
        try Aircraft.order(Aircraft.Columns.registration.asc).fetchAll(db)
      }
    } catch {
      print("LocalDatabase: Error fetching all aircraft: \(error)")
      return []
    }
  }
}
